...
struct SubtractGpuPipeline {
    pipeline: wgpu::ComputePipeline,
    bind_group_layout: wgpu::BindGroupLayout,
}

fn build_subtract_pipeline(gpu: &GpuState) -> SubtractGpuPipeline {
    let shader = gpu.create_shader(SUBTRACT_SHADER);
    ...
    let pipeline = gpu.create_compute_pipeline("subtract pipeline", &shader, "main", &[&bind_group_layout]);

    SubtractGpuPipeline { pipeline, bind_group_layout }
}

#[derive(Clone)]
struct SubtractFingerprint {
    foreground: Value,
    background: Value,
}

impl SubtractFingerprint {
    fn matches(&self, other: &SubtractFingerprint) -> bool {
        value_ptr_eq(&self.foreground, &other.foreground) && value_ptr_eq(&self.background, &other.background)
    }
}

struct CompletedSubtractJob {
    fingerprint: SubtractFingerprint,
    pixels: Vec<f32>,
    width: u32,
    height: u32,
}
...
pub struct Subtract {
    ...
    // full rationale.
    gpu_pipeline: RefCell<Option<SubtractGpuPipeline>>,
    pending: Rc<RefCell<Option<SubtractFingerprint>>>,
    last_gpu_result: Rc<RefCell<Option<CompletedSubtractJob>>>,
}
...
impl Subtract {
    ...
    fn dispatch_gpu(&self, gpu: Arc<GpuState>, fingerprint: SubtractFingerprint, foreground: FloatImage, background: FloatImage) {
...

        {
            ...
            if pipeline_slot.is_none() {
                *pipeline_slot = Some(build_subtract_pipeline(&gpu));
            }
        }

        let foreground_buffer = gpu.upload("subtract foreground", &foreground.pixels, wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC);
        let background_buffer = gpu.upload("subtract background", &background.pixels, wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC);
        ...
        let params_buffer = gpu.upload("subtract params", &params, wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST);
...

        {
            ...
            let bind_group = gpu.create_bind_group("subtract bind group", &pipeline.bind_group_layout, &[
            ...
            gpu.dispatch("subtract dispatch", &pipeline.pipeline, &bind_group, (workgroups_x, workgroups_y, 1));
        }
        ...
        {
            let pixels = match gpu.read_buffer_blocking(&readback_buffer, len) {
                Ok(result) => result,
                Err(_) => {
            ...
            *self.last_gpu_result.borrow_mut() = Some(CompletedSubtractJob { fingerprint, pixels, width, height });
            *self.pending.borrow_mut() = None;
        }
        ...
        #[cfg(target_arch = "wasm32")]
        {
            *self.pending.borrow_mut() = Some(fingerprint.clone());
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let pixels = match gpu.read_buffer_async(&readback_buffer, len).await {
                    Ok(result) => result,
                    Err(_) => {
                        // RFC-006 Fix 2: readback failure degrades to CPU fallback,
                ...
                *last_gpu_result.borrow_mut() = Some(CompletedSubtractJob {
...
impl Operation for Subtract {
    ...
    fn execute(&self, ctx: &Context, inputs: &[(Input, Value)]) -> Result<Vec<Value>, OperationError> {
        ...
        if mask.is_some() {
            ...
            let subtracted = crate::graphics::compute_within_bbox(width, first_image.height, work_area, a, |x, y| {
                Self::subtract_single_pixel(a, b, x, y, width)
            });
            let subtracted = crate::graphics::apply_mask(&first_image.pixels, subtracted, mask, first_image.width, first_image.height)?;

            return Ok(vec![Value::FloatImage(Arc::new(FloatImage { pixels: subtracted, width: first_image.width, height: first_image.height }))]);
        }

        // Unmasked path: try GPU first when available.
        if let Some(gpu) = ctx.gpu.clone() {
            let fingerprint = SubtractFingerprint { foreground: first.clone(), background: second.clone() };
...
mod tests {
    ...
    fn is_live_is_true_only_while_a_gpu_dispatch_is_pending() {
        let subtract = Subtract::new();
        assert!(!subtract.is_live());

        *subtract.pending.borrow_mut() = Some(SubtractFingerprint { foreground: Value::Number(0.0), background: Value::Number(0.0) });
        assert!(subtract.is_live());

        *subtract.pending.borrow_mut() = None;
        assert!(!subtract.is_live());
    }
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let subtract = Subtract::new();
        let already_in_flight = SubtractFingerprint { foreground: Value::Number(0.0), background: Value::Number(0.0) };
        *subtract.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 1) };
        let _ = subtract.execute(&ctx, &[(Input::Foreground, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1))), (Input::Background, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1)))]).unwrap();

        assert!(
            subtract.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
            "a second GPU dispatch must not be kicked off while one is already pending"
        );
    }

    #[test]
fn gpu_subtract_matches_cpu_within_tolerance_once_warmed_up() {
        let Ok(gpu) = pollster::block_on(crate::gpu::GpuState::new()) else {
            eprintln!("skipping: no GPU adapter available in this environment");
            return;
        };
        ...
        let cpu_subtract = Subtract::new();
        let cpu_values = cpu_subtract
        ...
        let gpu_subtract = Subtract::new();
        let gpu_ctx = Context { gpu: Some(gpu), ..context(width, height) };
        let _ = gpu_subtract.execute(&gpu_ctx, &[(Input::Foreground, Value::Image(fg.clone())), (Input::Background, Value::Image(bg.clone()))]).unwrap();
        let gpu_values = gpu_subtract.execute(&gpu_ctx, &[(Input::Foreground, Value::Image(fg)), (Input::Background, Value::Image(bg))]).unwrap();
