...
mod tests {
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let add = Add::new();
        let already_in_flight = AddFingerprint { foreground: Value::Number(0.0), background: Value::Number(0.0) };
        *add.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 1) };
        let _ = add.execute(&ctx, &[(Input::Foreground, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1))), (Input::Background, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1)))]).unwrap();

        assert!(
            add.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
