...
mod tests {
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let resize = Resize::new();
        let already_in_flight = ResizeFingerprint { source: Value::Number(0.0), scale_x_bits: 50.0_f64.to_bits(), scale_y_bits: 50.0_f64.to_bits() };
        *resize.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 2) };
        let _ = resize.execute(&ctx, &[(Input::Source, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255, 10, 20, 30, 255, 50, 60, 70, 255], 2, 2)))]).unwrap();

        assert!(
            resize.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
