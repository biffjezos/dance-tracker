...
mod tests {
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let shuffle = Shuffle::new();
        let already_in_flight = ShuffleFingerprint { source: Value::Number(0.0), red: ShuffleChannel::R, green: ShuffleChannel::G, blue: ShuffleChannel::B, alpha: ShuffleChannel::Off };
        *shuffle.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 1) };
        let _ = shuffle.execute(&ctx, &[(Input::Source, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1)))]).unwrap();

        assert!(
            shuffle.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
