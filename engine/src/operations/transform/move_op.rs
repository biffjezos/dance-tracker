...
mod tests {
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let mv = Move::new();
        let already_in_flight = MoveFingerprint { source: Value::Number(0.0), offset_x_bits: 1.0_f64.to_bits(), offset_y_bits: 0.0_f64.to_bits() };
        *mv.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 1) };
        let _ = mv.execute(&ctx, &[(Input::Source, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1)))]).unwrap();

        assert!(
            mv.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
