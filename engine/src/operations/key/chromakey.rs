...
mod tests {
    ...
    fn only_one_gpu_dispatch_stays_pending_while_input_keeps_changing() {
        ...
        let chromakey = ChromaKey::new();
        let already_in_flight = ChromaKeyFingerprint { source: Value::Number(0.0), key_color: Color { r: 0.0, g: 1.0, b: 0.0, a: 1.0 }, threshold: 0.3f64.to_bits() };
        *chromakey.pending.borrow_mut() = Some(already_in_flight.clone());

        let ctx = Context { gpu: Some(gpu), ..context(2, 1) };
        let _ = chromakey.execute(&ctx, &[(Input::Source, Value::Image(image(vec![10, 20, 30, 255, 50, 60, 70, 255], 2, 1)))]).unwrap();

        assert!(
            chromakey.pending.borrow().as_ref().unwrap().matches(&already_in_flight),
